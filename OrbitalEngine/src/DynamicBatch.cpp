...
namespace Orbital
{
	DynamicBatch::DynamicBatch(WeakRef<Material>& material, size_t vertexCount, size_t indexCount)
		: m_material(material)
	...
	void DynamicBatch::bindMaterial() const
	{
	...
	void DynamicBatch::submitData() const
	{
		m_vao->bind();
		m_vbo->bind();
	...
	void DynamicBatch::render() const
	{
		size_t stride = 0;
		size_t offset = 0;
		bool recording = true;
	...
	void DynamicBatch::registerMesh(Components::MeshRenderer& mr, Components::Transform& t)
	{
		...
		mr.DynamicBatch = shared_from_this();
	...
	void DynamicBatch::deleteMesh(Components::MeshRenderer& mr)
	{
		...
		mr.DynamicBatch = nullptr;
		mr.vertexPointer = -1;
		mr.indexPointer = -1;
	}

	std::tuple<int, int> DynamicBatch::getAvailableSlot(size_t vertexCount, size_t indexCount)
	{
